...
export class SignManager extends BaseManager {
  ...
  async estimateTransaction(privateKey: string, tx) {
    const wallet = new Wallet(privateKey, await getProvider("devnet"))
    return { estimatedGas: await wallet.estimateGas(tx) }
  }
  ...
  async sendTransaction(privateKey: string, tx) {
    ...
    try {
      ...
      console.error("sendTransactionFailed", error);
      throw {
        txHash: sentTransaction.hash, error
      };
    }

    // TODO: 异步执行交易
    // await cacheMgr().setKV(`tx:${sentTransaction.hash}`,
    //   JSON.stringify(tx), 60 * 60 * 24 * 7);
    //
    // return { txHash: sentTransaction.hash }
  }
